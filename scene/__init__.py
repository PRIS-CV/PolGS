...
class Scene:
    ...
    def __init__(self, args : ModelParams, gaussians : GaussianModel, camera_lr: float=0.0, load_iteration=None, shuffle=True, resolution_scales=[1.0]):
        ...
        if 'SMVP3D' in args.source_path and os.path.exists(os.path.join(args.source_path, "cameras.npz")):
            print("Found SMVP3D data format!")
            scene_info = sceneLoadTypeCallbacks["SMVP3D"](args.source_path, args.white_background, args.eval)
        elif os.path.exists(os.path.join(args.source_path, "cameras.npz")):
            print("Found cameras.npz file, assuming REALWORLD data format!")
            scene_info = sceneLoadTypeCallbacks["REALWORLD"](args.source_path, args.white_background, args.eval)
        else:
