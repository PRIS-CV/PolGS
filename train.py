...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    count = -1
    initial_stage = True
    viewpoint = scene.getTrainCameras(1).copy()[:]
    # viewpoint_scale = scene.getTrainCameras(4).copy()[:]
    # num_views = len(viewpoint)
    # num_id = 0
    for iteration in range(first_iter, opt.iterations + 2):
        ...
        # if mono is not None:
        #     if viewpoint_cam.HWK[0] == 512 and viewpoint_cam.HWK[1] == 512:
        #         normal_ = torch.cat([-normal[:1], -normal[1:2], normal[2:]])
        #         normal_ = ((normal2rgb(normal_, mask_gt)-0.5)*2)
        #     else:
        #         normal_ = ((normal2rgb(normal, mask_gt)-0.5)*2)
        #     loss_monoN = cos_loss(normal_, monoN, weight=mask_gt)
        ...
        with torch.no_grad():
            ...
            training_report(tb_writer, iteration, Ll1, loss, l1_loss, iter_start.elapsed_time(iter_end), 
                            testing_iterations, scene, pipe, test_background, use_mask, loss_stokes=loss_stokes)
...
def training_report(tb_writer, iteration, Ll1, loss, l1_loss, elapsed, testing_iterations, scene : Scene, pipe, bg, use_mask, loss_stokes = 0):
    if tb_writer:
    ...
    #     validation_configs = ({'name': 'train', 'cameras' : scene.getTrainCameras()[::8]})
